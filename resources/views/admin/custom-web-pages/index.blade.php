...
@section('main')
    <section class="container">
        <h2 class="fw-bold mb-2 text-uppercase">Custom Web Pages</h2>
        @if(count($pages))
                        ...
                        <tr id="row{{ $page->id }}">
                            ...
                            <td>
                                <form id="deleteForm{{ $page->id }}" method="POST" hidden
                                    action="{{ route('admin.custom-web-pages.destroy', ['custom_web_page' => $page]) }}">
                                    @csrf
                                    @method('DELETE')
                                </form>
                                <a href="{{ route('custom-web-page', ['pathname' => $page->pathname]) }}"
...
@push('after footer')
    @vite('resources/js/admin/customWebPages/index.js')
@endpush
